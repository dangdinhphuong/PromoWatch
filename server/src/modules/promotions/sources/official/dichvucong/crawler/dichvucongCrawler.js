  ...
  while (true) {
    ...
    try {
      ...
      const isConnectTimeout =
        causeCode === "UND_ERR_CONNECT_TIMEOUT" || causeName === "ConnectTimeoutError";

      if (isConnectTimeout) {
        logger.warn(
          `Connect timeout to dichvucong.gov.vn. Check network/VPN/firewall or set DVC_CRON_ENABLED=false.`
        );
      } else {
        logger.error(`Error fetching page ${pageIndex}: ${error?.message || String(error)}`);
        if (error?.cause) {
